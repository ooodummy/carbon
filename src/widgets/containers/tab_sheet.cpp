...
void carbon::tab_sheet::handle_draw() {
	...
	for (size_t i = 1; i < children.size(); i++) {
        ...
        // TODO: Icons should be by image or sprite references not using font images
		buf->draw_text(child->label_, { button.x + button.z / 2.0f, button.y + button.w / 2.0f }, COLOR_WHITE.alpha(child->animation_time_ * 255.0f), main_font, renderer::align_center);
	}
}
...
void carbon::tab_sheet::handle_input() {
	...
	for (size_t i = 1; i < children.size(); i++) {
		...
		const auto animation_speed = performance.get_dt() / 0.25f;
