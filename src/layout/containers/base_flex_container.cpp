...
void carbon::base_flex_container::decorate() {
    const auto margin_edge = margin_.get_edge();

    buf->draw_rect_filled(glm::vec2(margin_edge.x, margin_edge.y), glm::vec2(margin_edge.x + margin_edge.z, margin_edge.y + margin_edge.w), { 153, 93, 181 });
    buf->draw_rect(glm::vec2(margin_edge.x, margin_edge.y), glm::vec2(margin_edge.x + margin_edge.z, margin_edge.y + margin_edge.w), { 24, 26, 27 });
    buf->draw_rect_filled(glm::vec2(content_.x, content_.y), glm::vec2(content_.x + content_.z, content_.y + content_.w), {247, 148, 31});

    const auto margin = get_axes(margin_.get_edge());
...
void carbon::base_flex_container::set_flow(const flex_direction axis) {
	mark_dirty_and_propagate();
	flow_.main = axis;
	flow_.cross = (axis == row || axis == row_reversed) ? column : row;
}

void carbon::base_flex_container::set_flow(const flex_wrap wrap) {
	mark_dirty_and_propagate();
	flow_.wrap = wrap;
}

void carbon::base_flex_container::set_flow(const flex_direction axis, const flex_wrap wrap) {
	mark_dirty_and_propagate();
	flow_.main = axis;
	flow_.cross = (axis == row || axis == row_reversed) ? column : row;
	flow_.wrap = wrap;
...
glm::vec2 carbon::base_flex_container::get_main(const glm::vec4 src) const {
	return get_axis(src, flow_.main);
}

float carbon::base_flex_container::get_main(const glm::vec2 src) const {
	return get_axis(src, flow_.main);
}

glm::vec2 carbon::base_flex_container::get_cross(const glm::vec4 src) const {
	return get_axis(src, flow_.cross);
}

float carbon::base_flex_container::get_cross(const glm::vec2 src) const {
	return get_axis(src, flow_.cross);
}

void carbon::base_flex_container::set_main(glm::vec2& dst, const float src) const {
	if (flow_.main == row || flow_.main == row_reversed)
		dst.x = src;
	else
		dst.y = src;
