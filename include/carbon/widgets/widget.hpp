...
namespace carbon {
	...
	public:
        ...
        // Input callbacks
        virtual void on_mouse_move(const glm::vec2& position, const glm::vec2& delta) {};
        virtual void on_mouse_enter() {};
        virtual void on_mouse_leave() {};
        virtual void on_mouse_down(uint32_t button) {};
        virtual void on_mouse_up(uint32_t button) {};
        virtual void on_mouse_wheel(const glm::vec2& delta) {};
        virtual void on_key_down(uint32_t key) {};
        virtual void on_key_up(uint32_t key) {};

        bool is_input_locked() const {
            if (!context.active)
                return false;

            return !(window_ && window_->id == context.active->id);
        }
        ...
        void lock_input() {
            ...
            context.active = shared_from_this();
        }
	...
	public:
        ...
        // TODO: Remove child function although in most cases we can just disable the widget we might want to remove any if added by the scripting engine

		std::vector<std::shared_ptr<widget>> get_children() const;

        // Yoga wrapper functions
	...
	protected:
        ...
        widget* window_ = nullptr;
		widget* parent_ = nullptr;
		std::vector<std::shared_ptr<widget>> children_;

        // Should tooltips be managed as widgets to add to others, or should they just be a option in every widget?
